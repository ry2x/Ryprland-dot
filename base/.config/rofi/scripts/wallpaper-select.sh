...
thumbnail_size="384x252"
thumbnail_version="v7-${thumbnail_size}"

for command in find flock magick nproc rofi sha256sum stat theme-switch.sh wc xargs; do
...
rofi_override="textbox-gallery-count{content:\"${wallpaper_count} items\";}"
...
process_image() {
    ...
    cache_key=$(printf '%s\0%s\0%s' "$image" "$current_metadata" "$thumbnail_version" | sha256sum | cut -d' ' -f1)
    ...
    (
        ...
        if [[ ! -f "$cache_file" ]]; then
            if magick "${image}[0]" -strip -thumbnail "${thumbnail_size}^" -gravity center -extent "$thumbnail_size" "png:$cache_file.tmp"; then
...
export wallpaper_dir cache_dir thumbnail_size thumbnail_version
...
wall_selection=$(find "$wallpaper_dir" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.webp" -o -iname "*.gif" \) -printf '%P\n' |
    ...
    while IFS= read -r relative_path; do
        ...
        current_metadata=$(stat -Lc '%s:%Y' "$wallpaper_path")
        cache_key=$(printf '%s\0%s\0%s' "$wallpaper_path" "$current_metadata" "$thumbnail_version" | sha256sum | cut -d' ' -f1)
        cache_file="${cache_dir}/${cache_key}.png"
