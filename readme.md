...
stored below `${RYSTAL_SHELL_STATE_DIR}/theme`. Hyprland runs `theme-switch.sh refresh` after
...
Light mode uses `prefer-light`, `Adwaita`, and `breeze`; dark mode uses `prefer-dark`,
`Adwaita-dark`, and `breeze-dark`. The Matugen-generated `gtk.css` supplies the matching dynamic
surface and accent colors. Running GTK applications may receive the preference change live, but
should be restarted when they do not reload the generated CSS completely. `xdg-desktop-portal-gtk`
and `breeze-icons` must be installed.
...
Run the theme pipeline regression test without changing the active desktop:

```bash
node --test test/themeSwitcher.test.mjs
```
